...
(function() {
    ...
    // Конфигурация эффектов перков
    // Ключ: "skillIdx-perkIdx"
    // Значение: функция (step) => { бонусы }
    const perkEffects = {
        ...
        // Боевые искусства (3 ступени): +0/1/2 к кубам, +5/10/15 урона через броню
        "0-3": (step) => ({
            hitBonus: step - 1,
            armorPenetrationDamage: 5 * step
        }),
        // Мечник (3 ступени): силовая атака мечами +1/2/3 к кубам
        "0-4": (step) => ({ swordPowerAttackHitBonus: step }),
        // Контрудар: флаг (реализуется отдельно)
        "0-5": () => ({ counterAttackBonus: true }),
        // Копейщик (3 ступени): +5/10/15 кровопотеря, +5/10/15 через броню
        "0-6": (step) => ({
            spearBleedDamage: 5 * step,
            spearResistDamage: 5 * step
        }),
        // Рубака (3 ступени): +5/10/15 кровопотеря для топоров
        "0-7": (step) => ({ axeBleedDamage: 5 * step }),
        // Костолом (3 ступени): игнорирует 15/30/45% брони
        "0-8": (step) => ({ maceArmorPenetration: 0.15 * step }),
        // Двойной вихрь (2 ступени): +1/+2 к кубам для двух рук
        "0-9": (step) => ({ dualWieldHitBonus: step }),
        // Двойная мясорубка: силовые атаки с двух рук +50% урона
        "0-10": () => ({ dualWieldPowerAttackDamage: 0.50 }),
        // Безжалостный удар: силовая атака стоя +25% урона, 5% шанс отрубить голову
        "0-11": () => ({
        ...
        // Варвар (5 ступеней): +20% урона за ступень
        "1-0": (step) => ({ twoHandedDamage: 0.20 * step }),
        // Стойка чемпиона: +1 к кубу для силовых атак двуручным
        "1-1": () => ({ twoHandedPowerAttackHitBonus: 1 }),
        // Пикосажатель (3 ступени): +8/16/24 кровопотеря, +8/16/24 через броню
        "1-2": (step) => ({
            spearBleedDamage: 8 * step,
            spearResistDamage: 8 * step
        }),
        // Расчленитель (3 ступени): +10/20/30 кровопотеря для секир
        "1-3": (step) => ({ greatAxeBleedDamage: 10 * step }),
        // Глубокие раны (3 ступени): силовая атака мечами +1/2/3 к кубам
        "1-4": (step) => ({ greatswordPowerAttackHitBonus: step }),
        // Отшельник (3 ступени): +15/30/45% силы зачарования посоха, игнор 5/10/20% брони
        "1-5": (step) => ({
        ...
        // Сокрушительный удар: силовая атака стоя +25% урона, 10% шанс отрубить голову
        "1-7": () => ({
            standingPowerAttackDamage: 0.25,
    ...
    // Функция применения перков: сканирует чекбоксы и собирает бонусы
    ...
    // Функция для получения бонусов (используется в updateAll)
